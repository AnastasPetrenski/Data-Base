...
namespace ProductShop
{
    public class StartUp
    {
        ...
        public static void Main()
        {
            ...
            var inputUsersJSON = GetAllUsers(context, mapper);
            var resultDTO = ImportUserInfos(context, inputUsersJSON, mapper);
        }

        private static string GetAllUsers(ProductShopContext context, IMapper mapper)
        {
        ...
        private static string ImportUserInfos(ProductShopContext context, string inputJSON, IMapper mapper)
        {
            ...
            var objects = new List<UserInfo>();

            for (int i = 0; i < list.Count(); i++)
            {
                var token = list.ElementAt(i).Children();
                ...
                {
                    //JObject converter = JObject.FromObject(token[i]);
                    Console.WriteLine(value: token[i].Children()[j].ToString());
                }
            ...
            foreach (var token in list)
            {
                ...
                var serialize = JObject.FromObject(result);

            }

            context.UserInfos.AddRange(objects);
            context.SaveChanges();
        ...
        private static void ChangeUserAgeCategory(UserInfo result)
        {
            ...
            else if(result.Age < 130)
            {
                result.AgeCategory = Models.Enumerators.AgeCategory.Adult;
            }
            else
            {
                result.AgeCategory = Models.Enumerators.AgeCategory.NoCategory;
            }
